...
void conv_2d_buffer_cl_arr(
    hls::stream<data_arr_T> &data,
    // hls::stream<data_arr_T> (&data)[8],
    hls::stream<res_T>  &res,
    typename CONFIG_T::weight_t weights[CONFIG_T::filt_height * CONFIG_T::filt_width * CONFIG_T::n_chan * CONFIG_T::n_filt],
    typename CONFIG_T::bias_t   biases[CONFIG_T::n_filt])
{
    ...
    ReadInputArr: for (unsigned i = 0; i < 64; i++) {
        #pragma HLS PIPELINE II=CONFIG_T::reuse_factor
        data_arr = data.read();
        ReadInputSample: for (unsigned j = 0; j < 16; j++) {
            #pragma HLS LOOP_FLATTEN
            if(CONFIG_T::strategy == nnet::latency) {
                #pragma HLS PIPELINE II=CONFIG_T::reuse_factor
            }
            data_sample[0] = data_arr[j];
            compute_output_buffer_2d<data_T, res_T, CONFIG_T>(data_sample, line_buffer, res, weights, biases);
        }
    }
}
...
{
    #pragma HLS inline region
    conv_2d_buffer_cl_arr<data_T, data_arr_T, res_T, CONFIG_T>(data, res, weights, biases);
}
