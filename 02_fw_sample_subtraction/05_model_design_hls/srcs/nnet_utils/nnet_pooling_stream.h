...
template<class data_T, class data_arr_T, class res_T, typename CONFIG_T>
void pooling2d_avg_cl_parallel(
    hls::stream<data_arr_T> &data,
    hls::stream<res_T> &res
) {
    ...
    typename data_T::value_type kernel_data[CONFIG_T::pool_height * CONFIG_T::pool_width];
    #pragma HLS ARRAY_PARTITION variable = kernel_data complete dim = 0

    const int lShiftX = CONFIG_T::pool_width - 1;
    const int lShiftY = CONFIG_T::pool_height - 1;
    int pX = 0; // pixel X 
    int pY = 0; // pixel Y
    int sX = 0; // stride X
    int sY = 0; // stride Y

    input_arr_t data_arr;
    #pragma HLS DATA_PACK variable=data_arr

    input_t data_sample;
    #pragma HLS DATA_PACK variable=data_sample
    ...
    for (unsigned i = 0; i < 512; i++) {
        ...
        data_sample[0] = data_arr[i % 16];

        // Add pixel into line buffer, return pooling kernels
        nnet::shift_line_buffer<data_T, CONFIG_T>(data_sample, line_buffer, kernel_data);
        ...
        if ((sX - lShiftX) == 0 && (sY - lShiftY) == 0 && pY > lShiftY - 1 && pX > lShiftX - 1) {
            ...
            for(unsigned i_ihw = 0; i_ihw < CONFIG_T::pool_height * CONFIG_T::pool_width; i_ihw++) {
                pool_window[i_ihw] = kernel_data[i_ihw]; 
            }

            // Compute Pooling
            res_pack[0] = reduce_pool<typename CONFIG_T::accum_t, CONFIG_T::pool_height * CONFIG_T::pool_width, CONFIG_T>(pool_window);
