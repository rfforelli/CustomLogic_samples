...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open()) {
    while ( std::getline(fin,iline)) {
      ...
      unsigned num_frames_buffer = 4; // Number of frames the DRAM buffer should be able to hold
      unsigned num_groups_buffer = 4; // Number of groups the DRAM buffer should be able to hold
      // ap_uint<STREAM_DATA_WIDTH> DRAM[num_groups_buffer][(num_frames_buffer * IMAGE_HEIGHT * IMAGE_WIDTH * pixMono16::width) / STREAM_DATA_WIDTH];
      ap_uint<STREAM_DATA_WIDTH> DRAM[(num_groups_buffer * num_frames_buffer * IMAGE_HEIGHT * IMAGE_WIDTH * pixMono16::width) / STREAM_DATA_WIDTH];
      // ap_uint<STREAM_DATA_WIDTH> DRAM[600000];

      // void* DRAM = malloc(((num_groups_buffer * num_frames_buffer * IMAGE_HEIGHT * IMAGE_WIDTH * pixMono16::width) / STREAM_DATA_WIDTH) * 16);

      myproject_axi(VideoIn, VideoOut, &MetaIn, &MetaOut, (ap_uint<STREAM_DATA_WIDTH>*)DRAM); // Instantiate neural network
