...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open()) {
    while ( std::getline(fin,iline)) {
      ...
      unsigned num_photos_buffer = 12; // Number of photos the DRAM buffer should be able to hold
      ap_uint<STREAM_DATA_WIDTH> DRAM[(num_photos_buffer * IMAGE_HEIGHT * IMAGE_WIDTH * pixMono16::width) / STREAM_DATA_WIDTH];
