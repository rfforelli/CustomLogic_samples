 ...
 */
void pix_average(hls::stream<video_if_t> &StreamIn, hls::stream<video_if_t> &StreamOut, ap_uint<STREAM_DATA_WIDTH>* onboard_mem_base){

  static unsigned dram_wr_idx = 0; // valid address space is 0x0000000 to 0x40000000 (1 GB), so log2(0x40000000/STREAM_DATA_WIDTH) = 23 bits for the write index on Octo. 

  const unsigned PACKED_DEPTH = ((IMAGE_WIDTH * IMAGE_HEIGHT) / MONO16PIX_NBR); // 8 pixels per CoaXPress data packet

  const unsigned num_frames = 9; // Num frames to average

  static unsigned cntr = 0; // frame counter

  static ap_ufixed<16,16> sum[IMAGE_WIDTH * IMAGE_HEIGHT] = {0}; // Accumulator, sized to 16 bits to cover full range. MUST INCREASE IF num_frames INCREASES! Initialization unnecessary for hardware but useful for csim. 
  // #pragma HLS ARRAY_PARTITION variable=sum cyclic factor=8

  static video_if_t DataOut;
  video_if_t output_buf; // Output packet
  
  video_if_t temp; // Buffer packet
  ...
  for(unsigned i = 0; i < ((IMAGE_HEIGHT * IMAGE_WIDTH) / MONO16PIX_NBR); i++){ // Loop through the data stream These image dims MUST match eGrabber
    ...
    if(cntr < num_frames){ // Add frames up to n-2 to the running sum
      ...
      onboard_mem_base[dram_wr_idx] = DataOut.Data;
      dram_wr_idx++;
      ...
      for(unsigned j = 0; j < num_frames; j++){
        #pragma HLS UNROLL
        temp.Data = onboard_mem_base[i + (j*((IMAGE_HEIGHT * IMAGE_WIDTH) / MONO16PIX_NBR))]; // Grab the packet we want to add to the running sum
        for(unsigned k = 0; k < MONO16PIX_NBR; k++){
          #pragma HLS LOOP_FLATTEN
          sum[i*MONO16PIX_NBR + k] += temp.MONO16PIX(k);
        }
      }
      ...
      for(unsigned k = 0; k < MONO16PIX_NBR; k++){
        #pragma HLS UNROLL
        output_buf.MONO16PIX(k) = sum[i*MONO16PIX_NBR + k] / num_frames; // Calc average
        sum[i*MONO16PIX_NBR + k] = 0; // Reset running sum
      }
...
void myproject(
    ...
    pix_average(VideoBuffer, AveragedOut, onboard_mem_base); // Average every num_frames images
