...
template<class data_T, class res_T, class res_act_T , typename CONFIG_T>
void dense_relu(
    hls::stream<data_T> &data_stream,
    hls::stream<res_act_T>  &res_stream,
    typename CONFIG_T::weight_t weights[CONFIG_T::n_in*CONFIG_T::n_out],
    typename CONFIG_T::bias_t   biases[CONFIG_T::n_out])
{
    const typename res_T::value_type a = 0;
    ...
    ResWrite: for(unsigned i_out = 0; i_out < CONFIG_T::n_out / res_T::size; i_out++) {
        ...
        ResPack: for (int i_pack = 0; i_pack < res_T::size; i_pack++) {
            #pragma HLS UNROLL
            res_pack[i_pack] = (res[i_out * res_T::size + i_pack] > 0) ? res[i_out * res_T::size + i_pack] : a;
        }
        res_stream.write(res_pack);
    }
}
